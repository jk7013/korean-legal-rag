"""
세팅 5: Hybrid RAG - 벡터(bge-m3) + BM25 + RRF → LLM으로 풀기
"""
import os
import sys
...
from src.retrieval.hybrid_retriever import hybrid_evaluate_recall


def run_hybrid(embed_model: str = "bge-m3", llm_model: str = "gpt-4o-mini"):
    questions = load_questions_from_db()
    model_tag = f"hybrid-{embed_model}"
    ...
    for q in tqdm(todo, desc=f"[retrieved/{model_tag}/{llm_model}]"):
        retrieval = hybrid_evaluate_recall(
            q["question_text"],
...
if __name__ == "__main__":
    ...
    run_hybrid(args.embed_model, args.llm)
